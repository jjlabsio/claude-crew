...
[ ] A3a. 같은 목적으로 이미 사용 중인 라이브러리/프레임워크와 다른 것을 도입하는가? (도입하지 않으면 YES)
[ ] A3b. 프로젝트의 기존 디렉토리/모듈 구조 규칙을 위반하는가? (위반하지 않으면 YES)
...
- **Architect FAIL이고 NO 항목이 A4만이며, 근본 원인이 spec.md의 전제 조건이 현재 코드에서 불가능한 경우** (spec.md 결함):
  - Planner를 재시도해도 spec.md의 전제 조건을 고칠 수 없다.
  - 즉시 에스컬레이션:

```
Architect가 A4(전제 조건) 실패를 냈으며, 근본 원인은 spec.md의 전제 조건이 현재 코드 상태에서 불가능하기 때문입니다.
Planner 재시도로는 해결할 수 없습니다. spec.md를 수정해야 합니다.
[1] PM 에이전트를 재실행하여 spec.md를 재작성하시겠습니까?
[2] spec.md를 직접 수정하시겠습니까?
[3] 이 태스크를 보류하겠습니까?
...
| Phase 3a Architect spec.md 전제조건 결함 FAIL (A4만, spec 원인) | 즉시 에스컬레이션 | 1회 |
