...
.crew/plans/{task-id}/
  ...
  review-1.md       # 1회차 FAIL 아카이브 (1회차 실패 시 생성)
  review-2.md       # 2회차 FAIL 아카이브 (2회차 실패 시 생성)
...
[ ] A3. 구현 접근법이 기존 코드 패턴/아키텍처와 충돌하지 않는가?
...
[ ] C3. 커버리지: spec.md의 수용 기준 각각에 대해, plan.md의 태스크 목록에서 최소 하나의 태스크가 해당 기준을 직접 다루는가?
[ ] C4. 추적성: plan.md의 검증 방법 섹션이 spec.md의 수용 기준과 1:1 매핑을 명시적으로 작성했는가?
...
2. 오케스트레이터가 `contract.md`를 직접 작성한다.
...
- **Architect FAIL 또는 Critic FAIL(C3-C7 포함)인 경우** (plan.md 결함):
  - 피드백 보존 루프를 진행한다.
...
예시:
- 첫 번째 FAIL (카운터 = 0): `review.md` → `review-1.md`, n=1
- 두 번째 FAIL (카운터 = 1): `review.md` → `review-2.md`, n=2
- 세 번째 FAIL (카운터 = 2): `review.md` → `review-3.md`, n=3
- 네 번째 FAIL (카운터 = 3): `review.md` → `review-4.md`, n=4
...
Planner 에이전트를 retry 프롬프트로 실행한다. 입력에 `review-{n}.md`를 포함한다.
...
| Critic | sonnet | spec.md + plan.md | brief.md | 파일 읽기 |
...
{
  "status": "ESCALATE",
  "phase": "pre-1" | "1" | "2" | "3a-agent-fail" | "3a-arch-fail" | "3b-agent-fail" | "3b-spec-defect" | "3-overflow",
  "reason": "자유형 텍스트",
